...
module.exports = async (csn) => {
    ...
    // Add data products support
    const { processDataProducts } = require("./dataProducts");
    // Pass packages from ordDocument to processDataProducts
    const dataProductsAppConfig = { ...appConfig, packages: ordDocument.packages };
    const dataProducts = await processDataProducts(linkedCsn, dataProductsAppConfig);
    if (dataProducts.length > 0) {
        ordDocument.dataProducts = dataProducts;
    }
