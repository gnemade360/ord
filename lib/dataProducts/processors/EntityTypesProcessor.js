...
class EntityTypesProcessor extends BaseProcessor {
    ...
    enrichWithEntityTypes(dp, context) {
        ...
        // Check for explicit entity types annotation on service
        ...
        // Auto-discover entity types from service entities
        Object.entries(service.elements || {}).forEach(([elemName, elem]) => {
            if (elem.kind === 'entity') {
                // Check if the entity has an explicit entityType annotation
                const entity = csn.definitions[elem.target];
                if (entity && entity['@ORD.entityType']) {
                    entityTypes.add(entity['@ORD.entityType']);
                } else {
                    // Generate entity type ID based on namespace and entity name
        ...
        // Check for entity type references in associations
